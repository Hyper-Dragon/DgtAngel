...
function GetRemoteBoardState() {
	...
	try {
		if ((remoteBoard.PageUrl.includes("chess.com/game/live/") || remoteBoard.PageUrl.includes("chess.com/play"))) {
			if (Array.from(document.getElementsByClassName('new-game-focus-mode-time-selector')).length == 1) {
				remoteBoard.State.Code = "UNKNOWN_PAGE";
				remoteBoard.State.Message = "Do not use focus mode on the *Play* screen...there is no access to the last move!";
				...
				piecesStringOut = myPiecesArray.map(o => o.style.backgroundImage.split('/').pop().substring(0, 2) + o.className.replace("piece ", "").replace(" square-", "").replaceAll("0", "")).join(',');
				...
				if (moveList.length > 0) {
					lastMoveRow = Array.from(document.getElementsByClassName('move')).pop().innerText.trim().split("\n");
				...
				wcConTime = 0;
				whiteClock.innerText.split(":").reverse().forEach(element => wcConTime += w_mul.pop() * parseFloat(element.trim()))

				var b_mul = [3600000.0, 60000.0, 1000.0];
				bcConTime = 0;
				blackClock.innerText.split(":").reverse().forEach(element => bcConTime += b_mul.pop() * parseFloat(element.trim()))

				remoteBoard.Board.FenString = result;
				remoteBoard.Board.Turn = turn;
				remoteBoard.Board.IsWhiteOnBottom = whiteClock.classList.contains("clock-bottom");
				...
				if (remoteBoard.Board.Turn == "NONE") {
					...
					if (remoteBoard.Board.LastMove == "1-0" ||
